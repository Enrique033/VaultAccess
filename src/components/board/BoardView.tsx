    ...
    >
      <header className="flex items-center gap-2 px-3 py-2.5">
        ...
        <button
          ...
          className="shrink-0 rounded-lg p-1 text-muted transition-colors hover:bg-surface hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30"
        >
          <Plus className="size-4" />
        </button>
        {isRealCategory &&
          (onAddSubcategory || onRenameColumn || onDeleteColumn) && (
            <DropdownMenu
            ...
            >
              ...
              {onRenameColumn && (
                <DropdownMenuItem onClick={startRename}>
                  <Pencil className="size-3.5" /> Renombrar lista
                </DropdownMenuItem>
              )}
              {onDeleteColumn && (
                <>
...
function InlineAddButton({
  ...
  return (
    <div className="grid grid-rows-[0fr] transition-[grid-template-rows] duration-200 group-hover/column:grid-rows-[1fr] focus-within:grid-rows-[1fr]">
      <button
        type="button"
        tabIndex={-1}
        onClick={onClick}
        className="flex min-h-0 w-full items-center gap-1.5 overflow-hidden rounded-lg px-1.5 text-left text-[11px] font-medium text-muted opacity-0 transition-colors hover:bg-surface/80 hover:text-primary focus-visible:opacity-100 group-hover/column:opacity-100"
      >
